...
public class TransactionHistoryEventHandler {
    ...
    @EventHandler
    public void on(DepositedMoneyEvent event) throws TransactionHistory.TransactionHistoryCreateException {
        transactionHistoryRepository.save(new TransactionHistory(event.getAccountId(), event.getAmount(), event.getTransactionId()));
    }

    @EventHandler
    public void on(MoneyWithdrawnEvent event) throws TransactionHistory.TransactionHistoryCreateException {
        transactionHistoryRepository.save(new TransactionHistory(event.getAccountId(), -event.getAmount(), event.getTransactionId()));
    }
}
