 ...
 * 
 */
@Service("deployProjectHandler")
public class DeployProjectHandler extends CodeHandler {
	...
	public Map<String, Object> handle(MergeObejectEntity object,
		...
		String cxxfxPath = PlatData.FILEPATH+File.separator+"ts-dp-css-fx";
		...
		while (iterator.hasNext()) {
			...
			try {
				repo = new FileRepositoryBuilder().setGitDir(refile).build();
			} catch (Exception e2) {
				logger.error("org.eclipse.jgit.lib.Repository exception",e2);
			}
			Git git = new Git(repo);
			try {
				// 1,拉取最新的代码
				logger.info("1)拉取最新的代码");
				flag = pullNewCode(object, flag, path, git);
				// 2,修改pom文件和打包
				logger.info("2)修改pom文件和打包工程");
				flag = isRunFlag(object, flag, path, git);
				// 3,部署服务 4,提交代码
				logger.info("3)部署服务和4)提交代码");
				flag = deployProject(object, flag, path, git);
			} catch (Exception e) {
				try {
					git.reset().setMode(ResetType.HARD).setRef("HEAD").call();
					flag.put("flag", false);
					flag.put("msg：", "未知异常");
					logger.error("部署项目失败:" + object.getTarget().getName()
							+ "在工程::" + path, e);
				} catch (Exception e1) {
					logger.error(e1);
				}
	 ...
	 * @return
	 */
	private Map<String, Object> pullNewCode(MergeObejectEntity object,
		...
		try {
			...
			// clone
			try {
				Git.cloneRepository().setURI(object.getTarget().getWeb_url())
	...
	// 跑服务
	private Map<String, Object> isRunFlag(MergeObejectEntity object,
			Map<String, Object> flag, String path, Git git) {
		...
		if (flag.get("flag").equals(true)) {
			try {
				...
				for (Iterator it = dependenciesElement.elementIterator(); it
					...
					if (artiString.equals(object.getTarget().getName())) {
						runflag = true;
						String newversion = (String) flag.get("newVersion");
	 ...
	 * @return
	 */
	private Map<String, Object> deployProject(MergeObejectEntity object,
			Map<String, Object> flag, String path, Git git) {
		String title = object.getTarget().getName();
			...
			try {
				...
				if (path.startsWith("/home/ts/git/ts-dp")) {
					...
					if (dpname.endsWith("css")) {
						File runFileS = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						...
						File runFile = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						Process startprocess = runtime.exec("bash startup-css.sh",
								null, runFile);
						bufferedReader = new BufferedReader(new InputStreamReader(
						...
						File runFile = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						Process startprocess = runtime.exec("bash startup-fcs.sh",
								null, runFile);
						bufferedReader = new BufferedReader(new InputStreamReader(
								startprocess.getInputStream()));
						while ((outString = bufferedReader.readLine()) != null) {
						...
						File runFileS = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						Process startprocessS = runtime.exec("bash shutdown-mc.sh",
								null, runFileS);
						bufferedReader = new BufferedReader(new InputStreamReader(
								startprocessS.getInputStream()));
						while ((outString = bufferedReader.readLine()) != null) {
						...
						File runFile = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						Process startprocess = runtime.exec("bash startup-mc.sh",
								null, runFile);
						bufferedReader = new BufferedReader(new InputStreamReader(
								startprocess.getInputStream()));
						while ((outString = bufferedReader.readLine()) != null) {
						...
						File runFile = new File(path + File.separator + "target"
								+ File.separator + dpname + "-" + dpversion
								+ File.separator + "bin");
						Process startprocess = runtime.exec("bash startup-jss.sh",
								null, runFile);
						bufferedReader = new BufferedReader(new InputStreamReader(
								startprocess.getInputStream()));
						while ((outString = bufferedReader.readLine()) != null) {
					...
					logger.info("启动dw工程");
					Process startprocess = runtime.exec("bash startup.sh", null,
							runFile);
					bufferedReader = new BufferedReader(new InputStreamReader(
							startprocess.getInputStream()));
					while ((outString = bufferedReader.readLine()) != null) {
...
/*public static void main(String[] args) throws Exception {
	 ...
	 String pomPath = "C://Users//zrp//git//ts-s-log" + File.separator + "pom.xml";
		SAXReader reader = new SAXReader();
		Document document = reader.read(pomPath);
		Element rootElement = document.getRootElement();
	 map.put("rootElement",rootElement);
	 System.out.println(map.get("rootElement")!=null);
	 Element rootElement13 = (Element) map.get("rootElement");
	 map.remove("rootElement");
		String dpname = rootElement13.element("artifactId").getText();
		String dpversion = rootElement13.elementText("version");
